...
public abstract class TransactionType {
    ...
    private static final byte TYPE_FLOAT_MINING = 20; // jump some for easier nxt updating
    ...
    private static final byte SUBTYPE_FLOAT_MINING_REWARD_RECIPIENT_ASSIGNMENT = 0;
    ...
    public static TransactionType findTransactionType(byte type, byte subtype) {
        switch (type) {
            ...
            case TYPE_FLOAT_MINING:
            	switch (subtype) {
            		case SUBTYPE_FLOAT_MINING_REWARD_RECIPIENT_ASSIGNMENT:
            			return FloatMining.REWARD_RECIPIENT_ASSIGNMENT;
            		default:
            			return null;
            	}
    ...
    */
    ...
    public static abstract class FloatMining extends TransactionType {
    	
    	private FloatMining() {}
    	
    	@Override
    	public final byte getType() {
    		return TransactionType.TYPE_FLOAT_MINING;
    	}
    	...
    	public static final TransactionType REWARD_RECIPIENT_ASSIGNMENT = new FloatMining() {
    		
    		@Override
    		public final byte getSubtype() {
    			return TransactionType.SUBTYPE_FLOAT_MINING_REWARD_RECIPIENT_ASSIGNMENT;
    		}
    		
    		@Override
            Attachment.FloatMiningRewardRecipientAssignment parseAttachment(ByteBuffer buffer, byte transactionVersion) throws NxtException.NotValidException {
                return new Attachment.FloatMiningRewardRecipientAssignment(buffer, transactionVersion);
            }

            @Override
            Attachment.FloatMiningRewardRecipientAssignment parseAttachment(JSONObject attachmentData) throws NxtException.NotValidException {
                return new Attachment.FloatMiningRewardRecipientAssignment(attachmentData);
            }
            ...
            boolean isDuplicate(Transaction transaction, Map<TransactionType, Set<String>> duplicates) {
                ...
                return isDuplicate(FloatMining.REWARD_RECIPIENT_ASSIGNMENT, Convert.toUnsignedLong(transaction.getSenderId()), duplicates);
            }
    		...
    		void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
    			...
    			if(height < Constants.FLOAT_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK) {
    				throw new NxtException.NotCurrentlyValidException("Reward recipient assignment not allowed before block " + Constants.FLOAT_REWARD_RECIPIENT_ASSIGNMENT_START_BLOCK);
    			}
    		}
		...
		public final static TransactionType ESCROW_CREATION = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(transaction.getFeeNQT() < Constants.ONE_NXT) {
					throw new NxtException.NotValidException("Escrow transaction must have a fee at least 1 float");
				}
		...
		public final static TransactionType SUBSCRIPTION_SUBSCRIBE = new AdvancedPayment() {
			...
			void validateAttachment(Transaction transaction) throws NxtException.ValidationException {
				...
				if(attachment.getFrequency() == null ||
				   attachment.getFrequency().intValue() < Constants.FLOAT_SUBSCRIPTION_MIN_FREQ ||
				   attachment.getFrequency().intValue() > Constants.FLOAT_SUBSCRIPTION_MAX_FREQ) {
				...
				if(transaction.getAmountNQT() < Constants.ONE_NXT || transaction.getAmountNQT() > Constants.MAX_BALANCE_NQT) {
					throw new NxtException.NotValidException("Subscriptions must be at least one float");
				}
				if(transaction.getSenderId() == transaction.getRecipientId()) {
