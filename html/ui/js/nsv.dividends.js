var NSV = (function(NSV, $, undefined) {	
	...
	NSV.div_ad_gen = function () {
		...
		if (NRS.accountInfo.unconfirmedBalanceNQT) {
			...
			if (balance < len) {
				err_message = "You don't have enough FLOAT in this account for this distribution. ".concat(String(len),"FLOAT needed.");
			}
		}
		else {
	...
	NSV.getAccountError = function(accountId, callback) {
		NRS.sendRequest("getAccount", {
			...
			if (response.publicKey) {
				callback({
					"type": "info",
					"message": "The recipient account has a public key and a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " FLOAT.",
					"account": response
				});
			} else {
				if (response.errorCode) {
					if (response.errorCode == 4) {
						callback({
							"type": "danger",
							"message": "The recipient account is malformed, please adjust." + (!(/^(FLOAT\-)/i.test(accountId)) ? " If you want to type an alias, prepend it with the @ character." : ""),
							"account": null
						});
					...
					callback({
						"type": "warning",
						"message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + NRS.formatAmount(response.unconfirmedBalanceNQT, false, true) + " FLOAT. Please double check your recipient address before submitting.",
						"account": response
					});
				}
			}
	...
	NSV.div_send_gettime = function(inp_date) {
		...
		if (d1) {
			var float_gen = new Date("November 24, 2013, 12:00:00 UTC");
			var float_gen_sec = float_gen.getTime()/1000;
			var entry_time_sec = d1.getTime()/1000;
			var res = entry_time_sec - float_gen_sec; 
			return(res);
		} else {
			return d1;
		}
	...
	NSV.div_send_calc_dividends = function () {
		...
		if (!(/^\d+$/.test(asset))) {
			...
			NRS.sendOutsideRequest("/float?requestType=" + "getAsset", {
				"asset": asset
			}, function(response) {
				if (response.errorCode) {
				...
				if (outasset !== "") {				
					if (!(/^\d+$/.test(outasset))) {
						err_message = "Out Asset ID is invalid. Either enter a valid ID or leave blank to pay in Float.";
						...
						NRS.sendOutsideRequest("/float?requestType=" + "getAsset", {
							...
							if (response.errorCode) {
								err_message = "Incorrect Out Asset ID. Either enter a valid ID or leave blank to pay in Float.";	
							} else {				
								outasset_dec1 = response.decimals;
								outasset_name = response.name;
								NSV_div_send_out_float = false;
				...
				NRS.sendOutsideRequest("/float?requestType=" + "getTransaction", {"transaction": asset}, function(response) {				
					if (response.errorCode) {
				...
				NRS.sendOutsideRequest("/float?requestType=" + "getTime", {}, function(response) {
					if (response.errorCode) {
				...
				if (list_nodist_acc !== "") {
					if (list_nodist_acc !== "ISSUER") {
						...
						for (i=0; i<account_array.length; ++i) {
							var act_acc = account_array[i];
							NRS.sendOutsideRequest("/float?requestType=" + "getAccount", {
								"account": act_acc
							}, function(response) {
								if (response.errorCode) {
				...
				NRS.sendOutsideRequest("/float?requestType=" + "getTrades", {
					"asset": asset
				}, function(response) {
					if (response.errorCode) {
						...
						for (var i=0;i<trade_arr.length;i++) {
							if (trade_arr[i].timestamp <= activate_timestamp) {
								...
								NRS.sendOutsideRequest("/float?requestType=" + "getTransaction", {
				...
				if (NSV_div_send_out_float) {
					output = output.concat("Summary of proposed distribution of  ", amount, "FLOAT to ", String(aa_len),"\n");
				} else {
		...
		if (NSV_div_send_out_float) {
			...
			if (NRS.accountInfo.unconfirmedBalanceNQT) {
				...
				if (balance < (len*100000000)) {
					err_message = "You don't have enough Float in this account to pay the fees for this distribution. ".concat(String(len),"FLOAT needed.");
				}
			}
			else {
				err_message = "This account doesn't seem to have any funds.";
	...
	NSV.div_send_get_transactions = function(cur_asset, asset_array,initial_time,final_time) {
		...
		for (var i=0; i<len; i++) {
			...
			NRS.sendOutsideRequest("/float?requestType=" + "getAccountTransactionIds", {
				...
				if (response.errorCode) {
					...
					for (var j=0; j<tran_len; j++) {
						...
						NSV.sendOutsideRequest("/float?requestType=" + "getTransaction", {
								...
								if (response.timestamp <= final_time) {
									if (cur_asset == response.attachment.asset) {
										...
										if (response.senderRS == cur_account) {
											NSV_div_send_acc_array[cur_index].amount -= parseInt(response.attachment.quantityQNT,10);
											if (response.recipientRS != "FLOAT-NU58-Z4QR-XXKE-94DHH") {	//genesis (86281612813136630310)										
	...
	NSV.shareswap_calc = function() {
		...
		if (NSV_redeemed_assets.length === 0) {
			...
			if (num_tobe_swapped > 0) {
				out_message = out_message + "FLOAT needed for transactions: " + num_tobe_swapped.toString() + ", Replacement Assets needed: " + total_replacement_assets.toString() + "\n";
