...
var NRS = (function(NRS, $, undefined) {
		...
		if (alias) {
			...
			NRS.sendRequest("getAlias", {
				...
				if (response.errorCode) {
					...
					} else if ((aliasURI = /acct:(.*)@float/.exec(response.aliasURI)) || (aliasURI = /nacc:(.*)/.exec(response.aliasURI))) {
	...
	NRS.forms.setAlias = function($modal) {
		...
		if (data.type == "account") {
			if (!(/acct:(.*)@float/.test(data.aliasURI)) && !(/nacc:(.*)/.test(data.aliasURI))) {
				if (/^(FLOAT\-)/i.test(data.aliasURI)) {
					...
					if (!address.set(data.aliasURI)) {
						...
						data.aliasURI = "acct:" + data.aliasURI + "@float";
					}
	...
	function setAliasType(type, uri) {
		...
		if (type == "uri") {
			...
			$("#register_alias_uri").val("").mask("FLOAT-****-****-****-*****");

			if (uri) {
				var match = uri.match(/acct:(.*)@float/i);
				if (!match) {
				...
				} else if (!/^FLOAT\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{4}\-[A-Z0-9]{5}/i.test(uri)) {
					uri = NRS.accountRS;
				}
