...
$('#buy-ticket').on('click', function(e) {
	...
	$.ajax({
		url: 'http://' + window.location.hostname.toLowerCase() + ':' + window.location.port + '/float',
		type: 'POST',
		dataType: "json",
		data: jsonRequest,
...
$('#deploy-at-btn').on('click', function(e) {
	...
	var weeksA = $('#sel1').val()
	var floatsA = $('#funding-amount').val()
	var floatsA = floatsA * 100000000
	var passA = $('#funding-password').val()
	var feeA = 1000000000
	var minA = 700000000
	var hexA = Number(floatsA).toString(16).toLowerCase();
	var finalA = '';
	if ((hexA.length % 2) != 0) {
		hexA = "0" + hexA
	}
