...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.forms.addAssetBookmark = function($modal) {
		...
		if (!/^\d+$/.test(data.id) && !/^FLOAT\-/i.test(data.id)) {
			return {
				"error": $.t("error_asset_or_account_id_invalid")
			};
		}

		if (/^FLOAT\-/i.test(data.id)) {
	...
	$("#asset_order_modal").on("show.bs.modal", function(e) {
		...
		$("#asset_order_total").html(totalNXT + " FLOAT");
		$("#asset_order_fee_paid").html(NRS.formatAmount(feeNQT) + " FLOAT");
